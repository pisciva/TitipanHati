            ...
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                @php
                    $statusConfig = [
                        ...
                        'selesai_disalurkan' => ['class' => 'bg-green-400', 'icon' => 'fa-check-circle', 'text' => 'Selesai Disalurkan'],
                        'dibatalkan' => ['class' => 'bg-red-500', 'icon' => 'fa-times-circle', 'text' => 'Dibatalkan'],
                    ];
                    $config = $statusConfig[$donation->status] ?? ['class' => 'bg-gray-500', 'icon' => 'fa-info-circle', 'text' => ucfirst(str_replace('_', ' ', $donation->status))];
                @endphp
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        <ol class="relative border-l-2 border-gray-200 ml-4">
                            @foreach($donation->tracking->sortByDesc('status_changed_at') as $index => $track)
                            <li class="mb-6 ml-6 {{ $index === 0 ? 'pb-0' : '' }}">
                                <div class="absolute w-4 h-4 {{ $index === 0 ? 'bg-[#FF4400]' : 'bg-gray-300' }} rounded-full -left-2.5 border-4 border-white"></div>
                                <div class="bg-gray-50 rounded-xl p-4 {{ $index === 0 ? 'border-2 border-[#FF4400]' : 'border border-gray-200' }}">
                                    ...
                                    <h5 class="text-sm font-bold text-gray-900 flex items-center gap-2">
                                        @php
                                            $icons = [
                                                'menunggu_penjemputan' => 'fa-clock',
                                                'dalam_perjalanan' => 'fa-truck',
                                                'selesai_disalurkan' => 'fa-check-circle',
                                                'dibatalkan' => 'fa-times-circle',
                                            ];
                                            $icon = $icons[$track->status] ?? 'fa-info-circle';
                                        @endphp
                                        <i class="fas {{ $icon }} {{ $index === 0 ? 'text-[#FF4400]' : 'text-gray-600' }}"></i>
