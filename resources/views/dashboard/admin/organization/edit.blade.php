...
@section('title', 'Edit Organisasi: ' . $organization->name)

@section('content')

    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-[#1D1D1D]">Edit Organisasi</h1>
        <p class="text-sm text-gray-500">Ubah detail organisasi <span class="font-medium text-[#FF4400]">"{{ $organization->name }}"</span>.</p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
        ...
        <form action="{{ route('admin.organizations.update', $organization->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6"> 
                ...
                <div class="space-y-4 border-b pb-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  placeholder="Visi dan misi singkat organisasi...">{{ old('description', $organization->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="logo_url" class="block text-sm font-medium text-gray-700 mb-1">Logo Organisasi (Kosongkan jika tidak ingin diubah)</label>
                ...
                <div class="space-y-4 border-b pb-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="contact_email" id="contact_email" value="{{ old('contact_email', $organization->contact_email) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="4" required
                                  ...
                                  placeholder="Alamat kantor pusat organisasi...">{{ old('address', $organization->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="mt-1 flex space-x-6">
                            <label class="inline-flex items-center">
                                <input type="radio" name="is_verified" value="1" class="form-radio text-[#FF4400] h-4 w-4" 
                                    {{ old('is_verified', $organization->is_verified) == 1 ? 'checked' : '' }}>
                            ...
                            <label class="inline-flex items-center">
                                <input type="radio" name="is_verified" value="0" class="form-radio text-[#FF4400] h-4 w-4"
                                    {{ old('is_verified', $organization->is_verified) == 0 ? 'checked' : '' }}>
            ...
            <div class="mt-8 pt-4 border-t border-gray-200 flex justify-start space-x-3">
                ...
                <a href="{{ route('admin.organizations.index', $organization->id) }}" class="px-5 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                    Batal
                </a>
            </div>
        </form>
        ...
        function previewImage(event) {
            ...
            if (file) {
                ...
                @if($organization->logo_url)
                    preview.src = '{{ Storage::url($organization->logo_url) }}';
                @else
                    preview.src = 'https://via.placeholder.com/96x96?text=Preview'; 
                @endif
            }
        }
    </script>
